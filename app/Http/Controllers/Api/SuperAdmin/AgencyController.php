...
//============
    public function getSupplier(){
        $suplier = Agent::with(['legal_papers', 'country', 'city', 'zone', 'plan'])
        ->whereHas('plan', function ($plan) {
            $plan->where('type', 'suplier');
        })
        ->get();

    $data = $suplier->map(function ($agent) {
        return [
            'suplier_id' => $agent->id,
            'suplier_name' => $agent->name,
            'suplier_email' => $agent->email,
            'suplier_phone' => $agent->phone,
            'owner_name' => $agent->owner_name,
            'owner_phone' => $agent->owner_phone,
            'owner_email' => $agent->owner_email,
            'suplier_address' => $agent->address,
            'total_bookking' => $agent->total_booking,
            'total_commision' => $agent->total_commission,
            'country_id' => $agent->country_id,
            'country_name' => $agent->country?->name,
