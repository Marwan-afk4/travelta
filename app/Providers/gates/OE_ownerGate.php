...
class OE_ownerGate
{
    public static function defineGates()
    {
        ...
        Gate::define('view_OE_owner', function ($user) {
            ...
            $user->user_positions->perimitions->pluck('module')->contains('OE_owner') &&
            $user->user_positions->perimitions->pluck('action')->contains('view') ) {
                return true;
            }
            return false;
        });
        Gate::define('add_OE_owner', function ($user) {
            if ($user instanceof Agent || $user instanceof AffilateAgent) {
                return true;
            }
            if ($user->user_positions && 
        ...
        Gate::define('update_OE_owner', function ($user) {
            if ($user instanceof Agent || $user instanceof AffilateAgent) {
                return true;
            }
            if ($user->user_positions && 
        ...
        Gate::define('delete_OE_owner', function ($user) {
            if ($user instanceof Agent || $user instanceof AffilateAgent) {
                return true;
            }
            if ($user->user_positions && 
