...
class HRM_DepartmentGate
{
    public static function defineGates()
    {
        // if roles have booking payment module
        Gate::define('view_HRM_department', function ($user) {
            ...
            $user->user_positions->perimitions->pluck('module')->contains('HRM_department') &&
            $user->user_positions->perimitions->pluck('action')->contains('view') ) {
                return true;
            }
            return false;
        });
        Gate::define('add_HRM_department', function ($user) {
            if ($user instanceof Agent || $user instanceof AffilateAgent) {
                return true;
            }
            if ($user->user_positions && 
        ...
        Gate::define('update_HRM_department', function ($user) {
            if ($user instanceof Agent || $user instanceof AffilateAgent) {
                return true;
            }
            if ($user->user_positions && 
        ...
        Gate::define('delete_HRM_department', function ($user) {
            if ($user instanceof Agent || $user instanceof AffilateAgent) {
                return true;
            }
            if ($user->user_positions && 
